...
- Toegang van buitenaf: thuisnetwerk, VPN of Tailscale (SECURITY optie A/B; HTTPS via manier 1: eigen domein + reverse proxy van DSM op 443 (subnet route goedkeuren, lokale DNS-server + split DNS, toegangsprofiel met `100.64.0.0/10`), of manier 2: optioneel `tailscale serve` (bezet poort 443 van de ts.net-naam; verwijderen met `tailscale serve --https=443 off`), nooit `funnel`, geen exit node op de NAS i.v.m. Bunq-whitelist; firewall `100.64.0.0/10`). Eerste keer `tailscale serve`: eenmalige toestemmingslink ("Serve is not enabled on your tailnet") openen als tailnet-beheerder; controle met `tailscale serve status`.
...
- Inkomen: positief, geen terugbetaling, geen overboeking met eigen Bunq-rekening of eigen gekoppelde externe rekening (externe bankrekening: `isOwnExternalTransfer` op IBAN/account-id van niet-Bunq-rekeningen uit `/api/accounts`; blijft buiten de interne-overboekingsvlag, maar valt weg in `applyClientFilters` als het filter aan staat en altijd in budgetweergaven — geldt voor bij- én afschrijvingen). Backend `/api/statistics` idem (`extract_linked_external_ibans`, refunds verlagen uitgaven).
...
- `exclude_internal=true` filtering is aangescherpt:
  ...
  - linked external accounts (zoals externe bankrekening `MonetaryAccountExternal`) tellen expliciet niet als intern; Bunq `ExternalSavings` blijft wel intern.
  ...
  - balanswidgets voor betaal/spaar gebruiken nu alleen eigen Bunq-rekeningen (externe bankrekeningen vallen buiten `Betaalrekeningen (totaal)`).
  - widgetspecifiek:
    - `Top tegenrekeningen` en `Verdeling in categorieën` filteren nu ook expliciet interne/eigen tegenrekeningen weg op account-id, IBAN en eigen Bunq-rekeningnaam (zodat een tegenrekening op de eigen naam niet meer verschijnt).
