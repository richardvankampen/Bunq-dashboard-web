...
post_deploy_checks() {
  ...
  if printf '%s\n' "${LOG_OUTPUT}" | grep -q "Incorrect API key or IP address"; then
    ...
    say "     NO_PROMPT=true sh scripts/register_bunq_ip.sh ${SERVICE_NAME}"
    say "     # Optional explicit target override:"
    say "     TARGET_IP=<PUBLIEK_IPV4> NO_PROMPT=true sh scripts/register_bunq_ip.sh ${SERVICE_NAME}"
    exit 1
  fi
...
PY
  ...
  case "${CHECK_STATUS}" in
    ...
    MISMATCH)
      ...
      say "Run:"
      say "  TARGET_IP=${CHECK_EGRESS} NO_PROMPT=true sh scripts/register_bunq_ip.sh ${SERVICE_NAME}"
      exit 1
      ;;
    *)
